...
Why this exists:
  ...
  costs 3× LOW from the team's episode budget, so a team that always inflates
  ...
  numeric: it must reason about whether a HIGH claim is worth the 2× budget cost
...
#
# Calibration:
#   Average job: 4 hours × 1.5 avg multiplier = 6 units per job.
#   Budget of 200 → ~33 jobs at average priority.
#   Each team submits 1–2 jobs per window × 8 windows = 8–16 jobs.
#   Budget is intentionally ~2× expected demand so honest teams never run out,
#   while a team claiming CRITICAL on everything (3× multiplier) consumes
#   ~4–6h × 3.0 = 12–18 units/job → 200 / 15 ≈ 13 jobs max.
#
...
class ChargebackLedger:
    ...
    - Admitting a job charges: estimated_duration_hours × priority_multiplier
