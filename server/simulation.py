...
#   heat_in / (max_mass_flow x cp_air) = 441 / (50 x 1.006)  8.77 C
...
# headroom above heat-in: capacity / (mass_flow_max x 1.006 x REF)  1.06.
...
# 10x too high and dominated zone-level thermal behaviour unrealistically.
...
class FacilityState:
    ...
    def advance_load(self):
        ...
        Standard mode: diurnal curve x base_it_load_kw + random batch burst.
    ...
    def compute_free_cooling_potential(self) -> float:
        ...
        Rule of thumb: free cooling is viable when wet-bulb < (zone_temp - 4C).
        """
        if not self.zones:
            return 0.0
