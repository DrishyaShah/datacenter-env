...
class ZoneObservation(BaseModel):
    ...
    cold_aisle_temp_c: float = Field(
        ..., description="True cold-aisle supply temperature (°C)"
    )
    hot_aisle_temp_c: float = Field(
        ..., description="Return-air temperature from server exhausts (°C)"
    )
    reported_temp_c: float = Field(
        ..., description="Sensor-reported temperature — may include drift/fault offset (°C)"
    )
