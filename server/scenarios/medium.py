...
Episode: 30 steps x 24 min/step = 12 hours simulated time (condensed, step_scale=4.8).
...
def _medium_outside_temp_curve() -> list:
    ...
    for s in range(144):
        if s < 72:
            # 0 -> /2: smooth rise 18 -> 34
            frac = s / 72.0
            t = 18.0 + 16.0 * math.sin(frac * math.pi / 2.0)
        else:
            # /2 -> : 34 -> 28, slower decline
            frac = (s - 72) / 72.0
...
def build_medium_scenario(seed: int = 0) -> FacilityState:
    ...
    zone_ai = ZoneState(
        ...
        thermal_mass_kj_per_k=1133.0,  # 600/450 x 850
    )
    ...
    zone_storage = ZoneState(
        ...
        it_load_pct=0.60,
        thermal_mass_kj_per_k=378.0,   # 200/450 x 850
    )
    ...
    zone_infra = ZoneState(
        ...
        it_load_pct=0.60,
        thermal_mass_kj_per_k=283.0,   # 150/450 x 850
    )
